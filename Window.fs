...
type Window () =
    ...
    let camera = Camera (Vector2.zero, -4.0)
    let mutable visual =
    ...
    member this.Project (x : int, y : int) =
        ...
        view * vec2 (float x / float this.Width * 2.0 - 1.0) (1.0 - float y / float this.Height * 2.0)
    ...
    override this.OnRenderFrame args =
        ...
        visual.Render (Transform2.inverse this.Transform, &visual)
        this.SwapBuffers ()
